...
import { apiGet, apiPostJobs, PIPELINES, STYLES, consumeTrack } from "../api/client.js";
    ...
    return html`
      ...
      <div class="yb-card">
        ...
        <div class="yb-field-row">
          <yb-input label="Topic (tema)" .value=${this.topic} @yb-change=${(e) => (this.topic = e.detail.value)} placeholder="ej: Python tutorial"></yb-input>
          <yb-select label="Pipeline" .value=${this.pipeline} @yb-change=${(e) => (this.pipeline = e.detail.value)} .options=${PIPELINES.map((p) => ({ value: p, label: p }))}></yb-select>
        </div>
        <div class="yb-field-row">
          <yb-select label="Canción del catálogo (banda sonora)" .value=${this.track} @yb-change=${(e) => (this.track = e.detail.value)} .options=${trackOptions}></yb-select>
          <yb-input label="Duración (s)" .value=${this.duration} @yb-change=${(e) => (this.duration = e.detail.value)}></yb-input>
          <yb-input label="Resolución (opcional, WxH)" .value=${this.resolution} @yb-change=${(e) => (this.resolution = e.detail.value)} placeholder="1920x1080"></yb-input>
        </div>
        <div class="yb-field" style="flex-direction:row;align-items:center;gap:0.6rem">
          ...
          <label for="sync-toggle" style="margin:0">🎤 Sincronizar letras (--sync)</label>
        </div>
        ${this.sync
          ? html`
              <yb-select label="Estilo de subtítulos" .value=${this.style} @yb-change=${(e) => (this.style = e.detail.value)} .options=${STYLES.map((s) => ({ value: s, label: s }))}></yb-select>
            `
          : ""}
